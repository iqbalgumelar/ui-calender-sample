...
terraform {
  ...
  backend "s3" {
    ...
    bucket                 = "mysiloam-uat-terraform-state" #"<Your Bucket Name>"
    key                    = "terraform-states/svc-Mysiloam-uat-ecs-msm-fe-calendar/terraform.tfstate"
    ...
    dynamodb_table = "mysiloam-uat-terraform-state" #"<Your Dynamo DB>"
    encrypt        = true
  }
...
data "terraform_remote_state" "infrastructure" {
  ...
  config = {
    bucket = "mysiloam-uat-terraform-state" #"<Your Bucket Name>"
    key    = "terraform-states/mysiloam-uat-terraform-state/terraform.tfstate"
    region = var.region #"<Your Region>"
